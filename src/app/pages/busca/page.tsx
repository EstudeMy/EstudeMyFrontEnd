...
function BuscaContent() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    let url = `/pages/busca?q=${encodeURIComponent(termoBusca.trim())}`;
      ...
      <div className="relative z-10">
        <div className="flex flex-col min-h-screen">
          ...
          <div className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
            ...
            <div className="mb-8 bg-[var(--bg-card)] p-4 rounded-lg shadow-md border border-[var(--border-color)] transition-colors duration-300" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                {/* Alterar o div de busca para envolver em telas pequenas (md:flex-nowrap) */}
                <div className="flex flex-wrap md:flex-nowrap gap-2 items-center">
                  <input
                    ...
                    className="flex-1 w-full md:w-auto px-4 py-2 border border-[var(--border-color)] rounded-lg bg-[var(--bg-input)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-300"
                    style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)', borderColor: 'var(--border-color)', maxWidth:"100%"}}
                  />
                  <button
                    type="submit"
                    className="w-full md:w-auto bg-blue-600 text-white px-6 py-2 hover:bg-blue-700 dark:hover:bg-blue-500 transition rounded-3"
                  >
                    Buscar
                  </button>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <select
                    ...
                    className="flex-1 px-4 py-2 border border-[var(--border-color)] rounded-lg bg-[var(--bg-input)] text-[var(--text-primary)] focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm transition-colors duration-300"
                    style={{ backgroundColor: 'var(--bg-input)', color: 'var(--text-primary)', borderColor: 'var(--border-color)', maxWidth:"200px", minWidth:"200px"}}
                  >
            ...
            {loading ? (
              <div className="text-center py-12">
                <p className="text-[var(--text-secondary)]">Buscando...</p>
              </div>
            ) : error ? (
              <div className="text-center py-12">
              ...
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                  {trilhas.map((trilha) => (
                    <Link
                      key={trilha._id}
                      href={`/pages/trilha?trilhaId=${trilha._id}`}
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        {trilha.pagamento === "Paga" && (
                          <span className="inline-block mt-2 px-2 py-1 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 text-xs rounded transition-colors duration-300">
                            Paga
                          </span>
                        )}
                      </div>
